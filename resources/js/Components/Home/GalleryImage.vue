...
const handleImageChange = async (event) => {
    ...
    try {
        ...
        console.log('Subiendo imagen:', { imageName, action, section: props.section });
...
<template>
    <div class="relative w-full h-full">
        ...
        <img 
            :src="imageUrl" 
            :alt="`${section} - Imagen ${imageIndex + 1}`" 
            class="w-full h-full object-cover rounded-lg"
        />
        
        <!-- Overlay de edición -->
        <div v-if="isEditing" class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-200 rounded-lg">
